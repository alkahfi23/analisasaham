...
# PAGE
# =====================================================
st.set_page_config("IDX PRO Scanner", layout="wide")
st.title("📈 IDX PRO Scanner — Akumulasi Kuat")
...
IDX_SYMBOLS = [
    "IDX:BBRI","IDX:BMRI","IDX:BBCA","IDX:TLKM","IDX:ASII"
]
...
class TradingViewData:
    ...
    def get_ohlcv(self, symbol, tf, limit=200):
        r = self.session.post(
            ...
            json={"symbol":symbol,"resolution":TV_TF_MAP[tf],"count":limit},
            timeout=15
        )
        r.raise_for_status()
        d = r.json()
    ...
    def get_price(self, symbol):
        ...
        r.raise_for_status()
        return float(r.json()["data"][0]["d"]["lp"])
...
def reject_ara_arb(df):
    pc=df.close.iloc[-2]; lc=df.close.iloc[-1]
    ...
    if st.button("🔍 Scan IDX"):
        ...
        for s in IDX_SYMBOLS:
            try:
                ...
                adl=accumulation_distribution(df4h)
                if vo.iloc[-1]<VO_MIN or adl.iloc[-1]<=adl.iloc[-10]: continue
...
with tab2:
    st.subheader("🎲 Monte Carlo — Akumulasi Kuat")
    dummy_R=[-1,-1,0.8,2.0,0.8,-1,2.0,0.8]
    initial=st.number_input("Modal Awal",100_000_000,step=10_000_000)
    risk=st.slider("Risk / Trade (%)",0.25,1.0,0.5)/100
    trades=st.slider("Trades / Sim",50,300,150)
